...
export default class MapUtils {
  static async getGeocode(
    ...
    const geocode: google.maps.GeocoderResult | null = null;

    try {
      const response = await axios.get<google.maps.GeocoderResponse>(
  ...
  static drawPolygonCanvas(coordinates: Coordinates, canvasId: string) {
    ...
    const [
      ...
      selectedAreaMaxX,
      selectedAreaMaxY,
    ] = this.getPolygonBoundingBox(coordinates);
  ...
  static isContainPolygon(area: Coordinates, coord: Coordinate): boolean {
    ...
    for (let i = 1; i <= N; i++) {
      ...
      if (
        ...
        const xinters =
          ((coord.lat - p1.lat) * (p2.lng - p1.lng)) / (p2.lat - p1.lat) +
          p1.lng;
