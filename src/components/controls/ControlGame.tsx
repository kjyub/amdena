...
import { GameTypes } from "@/types/ControlTypes";
...
const Game = ({ type, currentType, setRandomType, title, icon }: IGame) => {
  ...
  return (
    ...
    >
      <div
        className={`icon bg-blue-400 border-2 border-blue-500 text-2xl text-yellow-400 font-bold shadow-yellow-500 ${isHighlight ? "shadow-game_icon" : ""}`}
      >
        {icon}
      </div>
